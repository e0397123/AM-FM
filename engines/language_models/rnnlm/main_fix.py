...
def read_train_data(filename):
    ...
    mask = []
    for item in data:
        temp_line = ['<pad>'] * FLAGS.seq_len
        temp_mask = [0.0] * FLAGS.seq_len
        if len(item) <= FLAGS.seq_len:
...
class TrainDataGeneratorSeq(object):
...
if __name__ == "__main__":
    ...
    test_words = read_data(os.path.join(dir_name, filenames[2]))
    ...
    if FLAGS.do_dstc_eval:
        ...
        hyp_list = read_data(os.path.join(dir_name, 'hyp_clean.txt'))
        splitted_hyp = []
        temp = []
